...
def run_experiment():
    ...
    log_path = os.path.join(config.RESULTS_DIR, "blocking_results.csv")
    
    v = {'name': 'D2', 'fields': ['business_name', 'business_address'], 'ngrams': (3,5)}
    
    print(f"\nRunning Variant {v['name']} - Fields: {v['fields']}, Ngrams: {v['ngrams']}")
    
    # Use config cache dir
    cache_dir = os.path.join(config.CACHE_DIR, f"tfidf_cache_exp002b_{v['name']}")
    blocker = TfidfBlocker(text_fields=v['fields'], ngram_range=v['ngrams'], cache_dir=cache_dir)
    ...
    print(f"Generating Candidates for {len(val_s1)} queries...")
    gen_start = time.time()
    ...
    log_df = pd.DataFrame([{
        'experiment_id': 'EXP-002B-stage2-aws',
        ...
        'runtime': total_time,
        'memory': 'AWS Optimized',
        'notes': "Full validation split benchmark on AWS"
    }])
    ...
    # Save candidates
    output_dir = os.path.join(config.REPO_ROOT, "artifacts", "blocking")
